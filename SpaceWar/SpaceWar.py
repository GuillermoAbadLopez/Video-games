...
class Sprite(turtle.Turtle):
    ...
    def is_collision(self, other):
        ...
        if (self.xcor() >= (other.xcor()-radi_col)) and \
        (self.xcor() <= (other.xcor()+radi_col)) and \
        (self.ycor() >= (other.ycor()-radi_col)) and \
        (self.ycor() <= (other.ycor()+radi_col)):
            return True
        else:
            return False
...
#NORMALSHOOT        
    def fire(self):
        if (player.special != "Berserk"):
            ...
            if (missile[0].status == "ready") and (missile[1].status == "ready") and (missile[2].status == "ready") and (missile[3].status == "ready"): 
...
#RESPAWN        
    while (player.status == "dead"):
        ...
        for enemy_number in range(number_enemies):
            if (enemy[enemy_number].xcor() < -200) and (enemy[enemy_number].ycor() < 100) and (enemy[enemy_number].ycor() > -100):
                xcoor = random.randint(-290,290)
                ycoor = random.randint(-290,290)
...
#POWERBOOST   
        if (powerboost.status == "available"):
            if player.is_collision(powerboost):
                ...
                winsound.PlaySound("BOOST.wav",  winsound.SND_ASYNC)
                #os.system("afplay BAD.wav&")                         #MAC
        ...
        if (powerboost.status == "active"):
            ...
            if (counter >= boost_time): 
                ...
                winsound.PlaySound("NO_BAD.wav",  winsound.SND_ASYNC)
                #os.system("afplay NO_BAD.wav&")                         #MAC
            ...
            if player.is_collision(enemy[enemy_number]):
                if (player.special != "Phantom") and (player.special != "Berserk"):
                    ...
                    if (game.lives!=0):
                        ...
                        #os.system("afplay MUERTE.wav&")                        #MAC
                    else:
                         winsound.PlaySound("GAME_OVER.wav",  winsound.SND_ASYNC)
                         #os.system("afplay GAME_OVER.wav&")                         #MAC
